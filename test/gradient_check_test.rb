...
def _uniform(*shape)
  return Cumo::SFloat.new(shape).rand(2) - 1
end
...
class NumericalGradientInvalidType < Test::Unit::TestCase
  def _setup()
    @x = Cumo::NArray.cast(0)
    @y = Cumo::NArray.cast(0)
    @f = lambda{}
  end
...
class NumericalGradientEpsTest < Test::Unit::TestCase
  def _setup()
    @x = Cumo::SFloat.cast(0.0)
    @y = Cumo::SFloat.cast(1.0)
  end
...
class TestCheckBackward < Test::Unit::TestCase
  ...
  def test_no_grads_for_not_float()
    x1 = Cumo::DFloat.cast([1])
    x2 = Cumo::Int32.cast([0, 1])
    g1 = Cumo::DFloat.cast([1])
    f = lambda do |x, y|
      s = Ident.new().(x)
      return [s]
    end
